...
def check_output(context):
    with open("tests/springboot/test_service_data_method.txt", "r", encoding="utf-8") as file:
        expected_output = file.read()
...
def check_output(context):
    with open("tests/springboot/test_service_method_only.txt", "r", encoding="utf-8") as file:
        expected_output = file.read()
    assert context["output"].replace(" ", "").replace("\n", "").strip() == expected_output.replace(" ", "").replace("\n", "").strip()
